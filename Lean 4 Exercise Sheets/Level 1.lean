...
example : Q → (P ∨ Q) := by --Here, ∨ is in the hypothesis. You "cases'" again. Now you have two goals with two different hypothesis.
sorry
done


example : (P ∨ Q) → (Q ∨ P) := by --Think about when you want to use "left" and when you want to use "right".
sorry
done
